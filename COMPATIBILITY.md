...
| **（GMA 25 支援版，版本號待定）** | **25.0 以上（至最新）** | 對齊現代 GMA。改用 `com.google.android.gms.ads.VersionInfo`。**要求 app `minSdkVersion` ≥ 23**（GMA 24 起的要求）。 |
...
> 註：媒體長寬比（aspectRatio）方向修正屬另一次的 BREAKING 變更，見該版 release note；與本頁 GMA 版本相容性是不同議題。
